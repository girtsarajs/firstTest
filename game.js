...
const REFLECT_OFFSET = 2; // reflect 2px before mirror center

let x = BORDER_PADDING + Math.random() * (WIDTH - 2 * BORDER_PADDING);
let y = BORDER_PADDING + Math.random() * (HEIGHT - 2 * BORDER_PADDING);
...
function generateMirrorPosition(existingMirrors) {
  ...
  return { x: mx, y: my, angle: Math.random() * 2 * Math.PI };
}
